...
<p>In this article, we will go a step further and augment Analog with some other tools to create a complete, incredibly powerful application stack that is typesafe from the database to the DOM rendering\xA0template. Thankfully, Angular&#39;s TypeScript support is unrivaled as the framework has been developed entirely in TypeScript since its initial release became available more than ten years ago. To maintain this high focus on typesafety throughout the entire application is only natural. With both client and server written in TypeScript, Analog\xA0puts us in an exceptional position to fully capitalize on this, especially as more and more\xA0fantastic server side capabilities become available\xA0in the Typescript ecosystem.</p>
...
<p>Nx is a next-generation build system that offers\xA0 first-class monorepo support and\xA0strong integrations for all well-known javascript frameworks. It enables you to use \xA0frontend frameworks like Angular or backend frameworks like NestJs\xA0to architect, test, and build your project at any size.</p>
...
\u2714 Choose your style                     \xB7 integrated
\u2714 What to create <span class="token keyword">in</span> the new workspace   \xB7 angular
\u2714 Repository name                       \xB7 spartan
\u2714 Application name                      \xB7 app
\u2714 Default stylesheet <span class="token function">format</span>             \xB7 css
\u2714 Enable distributed caching to <span class="token function">make</span> your CI faster \xB7 No</code></pre><p>Please note that all the commands are supposed to illustrate a basic setup. There\u2019s a lot of fine tuning necessary to get the application up and running. Cloning the GitHub repository is the fastest way to get started.</p>
...
Analog comes with SSG support. It makes it simple for you to export your website as basic static assets that you can host on GitHub Pages, S3, or any other static file server! Whether you choose to go fully static or leverage the benefits of server side rendering, Analog offers all of the SSR benefits for SEO and performance today. Still, there is even\xA0more. With Angular 16, we already\xA0get non-destructive hydration and\xA0significantly\xA0improved\xA0web vitals enhancements. You can expect even\xA0greater performance once zoneless\xA0applications powered by\xA0signals\xA0are released in version 17. Building on Analog puts you in a great position to take full advantage of those features.</p>
<p>Last but not least, Analog supports API (server) routes and file-based routing for Angular apps. In an analog application, the folder hierarchy and filenames determine the routing. You won&#39;t ever need to declare a route array again. Don&#39;t worry, you can still use the full power of the Angular router with its route guards and resolvers by defining route metadata\xA0within your files.</p>
<h3 id="tailwindcss">TailwindCSS</h3>
<p>Every good application needs a great UI. Although it&#39;s easier said than done, there are tools that help us do our best work. Tailwind is one of those essential\xA0tools for me.
...
<p>At this point our frontend is ready to go! However, it turns out that Analog sneakily turned us into a full stack developer. Code that we put inside the server folder is actually executed on the server \u{1F92F} It\u2019s that easy! Well let\u2019s take full advantage of our API being built in the same language and the same repository. Let\u2019s make our server client interaction super smooth and super typesafe with tRPC.</p>
...
<p>Prisma is a Node.js and TypeScript ORM. An ORM is an Object Relational Mapper, which is a piece of code that wraps the code required to manipulate the data, allowing you to avoid using SQL and instead interact directly with an object in the same language you&#39;re using. Hibernate is a popular ORM for\xA0Java and the\xA0Spring framework, .NET has the entity framework,\xA0and Typescript has Prisma. Prisma, however, goes beyond just a simple ORM. It has an intuitive data model, automated\xA0migrations (I can&#39;t emphasize how valuable\xA0this is), typesafety, and auto-completion!</p>
<p>With Prisma, you\xA0include a definition of your database schema\xA0within your code repository.
Prisma\xA0then creates a properly typed\xA0Client using this information. The Prisma client is query builder that is specific to your database schema and is aware of every one of your tables, their fields, and their relationships! With Prisma, we are literally typesafe from the database to the template.</p>
<p>Additionally, it creates SQL migrations automatically using the Prisma schema, monitors the execution of migrations, and offers tools for identifying and resolving conflicts and drifts between migrations and the\xA0database schema. This completely changes the game as it enables your database and all of its types to evolve alongside your application\xA0code.</p>
...
<p>PostgreSQL is one of the worlds most scalable databases. It is a sophisticated object-relational system applying\xA0SQL. Postgres allows you to securely store vast quantities of complex data. It enables developers to build the most sophisticated programs, carry out administrative operations, and establish integral\xA0environments. It is an open source technology trusted by millions of developers.</p>
<p>It turns out that every Supabase project is powered by a dedicated PostgreSQL database. Supabase is a fantastic open source Firebase replacement that is powered by the aforementioned Postgres database. It also provides a lot more features\xA0as your project expands and becomes more involved. Supabase provides solutions for common needs like as authentication, instant\xA0APIs, edge functions, real-time subscriptions, and storage, making it a robust platform on which to develop your application.</p>
<p>Even better, Supabase is open source at heart. You can look through the <a href="https://github.com/supabase/supabase">source code.</a> You may self-host it, utilize their free plan to get started, or commit to one of their premium\xA0subscriptions\xA0to receive a fully managed production quality environment that scales with your needs.</p>
<p>Supabase just wrapped up their <a href="https://supabase.com/launch-week">Launch Week 7</a> with a ton of incredible features. This\xA0includes\xA0an AI assistant that was\xA0added to their premium platform.\xA0It is aware of your database design and can help you when creating\xA0more complex SQL queries. I&#39;m even more thrilled by their most recent releases of outstanding open source projects, such as\xA0the PostgreSQL package manager <a href="https://database.dev">database.dev.</a> dbdev\xA0serves the same purpose for PostgreSQL that npm does for JavaScript. It\xA0gives your PostgresSQL database incredibly easy access to packages that give your DB\xA0superpowers like full-text search or one-time-only-read\xA0data access \xE0 la Snapchat.  The best thing is that dbdev can load any PostgreSQL instance that has the required fundamental extensions, independent of the Supabase platform. Again, the code is <a href="https://github.com/supabase/dbdev">open source.</a> I can&#39;t wait to see where this project goes in the future, and I have no doubt that we will soon be able to utilize a lot more fantastic libraries. Like Brandon Roberts puts it:</p>
...
export { _20230428SpartanTypeSafeAngularFullStackDevelopmentPoweredByAnalog as default };
//# sourceMappingURL=2023-04-28-spartan-type-safe-angular-full-stack-development-powered-by-analog-AUkUEWwZ.mjs.map
